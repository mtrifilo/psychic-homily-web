...
import type { ScrapedEvent, PreviewEvent, Scraper } from './types'
...
// TicketWeb scraper implementation
export const ticketwebScraper: Scraper = {
  ...
  async scrape(venueSlug: string, eventIds: string[]): Promise<ScrapedEvent[]> {
    const venue = VENUES[venueSlug]
    if (!venue) {
      throw new Error(`Unknown venue: ${venueSlug}`)
    }
