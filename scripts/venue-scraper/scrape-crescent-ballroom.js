...
async function scrapeCrescentBallroom() {
  console.log('Launching browser...');
  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();

  try {
    console.log('Navigating to Crescent Ballroom calendar...');
    await page.goto('https://www.crescentphx.com/calendar/', {
    ...
    const processedEvents = events.map(event => {
      ...
      return {
        ...
        date: event.start,
        venue: stripHtml(event.venue) || 'Crescent Ballroom',
