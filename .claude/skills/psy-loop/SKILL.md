...
- **Review tier by priority** (same memory for P2/P3; P4 is deliberately tightened here): P2, security, or data-integrity = full fresh-panel adversarial review, up to 3 rounds. P3 = one fresh four-lens round. P4 = gates plus ONE fresh single-lens review (Saboteur for logic, Future-Maintainer for docs), which is what earns the pass marker; the memory's "single inline lens" is the floor, and this loop raises it because an inline self-check shares the author's blind spots. The `ADVERSARIAL_REVIEW_SKIP=1` hatch cannot work as a per-command prefix anywhere: the hook reads that variable from its own process environment, which never sees a prefix on the gated command. Do not plan on it; earn the marker.
- **Interface changes and the full Go suite.** `feedback_interface_change_full_suite.md` says run `go test ./...` locally before pushing. Inside a loop that is overridden (see "Suites: who runs what"): the full suite does not fit the Bash tool's 600 s foreground ceiling and poisons sibling slots. Agents run the importer-scoped suites plus the memory's stub sweep (`grep -rn "stub<Name>\|var _ contracts.<Interface>" backend/`) and the full suite runs in CI.
- **`/code-review` is user-invocable only** (memory `pattern_code_review_not_agent_invocable`). Agents run `/simplify` in its place and the PR body's Code review section says so; `psy-dispatch` rule 3's "`/code-review` before every PR" is satisfied that way, with the gap disclosed, never implied to have run.

Everything else in `psy-dispatch` (pre-flight, isolation) and `adversarial-review` (finding bar, marker rules) applies unchanged. `psy-dispatch`'s take-over flow applies except for its `ADVERSARIAL_REVIEW_SKIP` step, which is superseded by the take-over paragraph under "Conflict shepherding".
...
| Keeps N slots full; never exceeds N. A slot frees when its agent REPORTS, not when the PR merges; conflict shepherding on an unmerged PR reuses the original agent and does not take a slot | Sets N (default 2; 3 when the backlog is growing faster than it shrinks) |
...
1. Copy `references/agent-rules-template.md` from this skill to the scratchpad as `agent-rules-<project-slug>-<YYYY-MM-DD>.md` (the scratchpad is shared across parallel sessions; a fixed name gets overwritten by another loop and stamps its session URL into your commits). Replace BOTH loop-wide placeholders: `<SESSION_URL>` (this session's `https://claude.ai/code/session_...` URL) and `<MODEL_NAME>` (the display name of the model the agents are dispatched on, e.g. `Claude Opus 5` when dispatching with `model: "opus"`; the trailer credits the model that wrote the commit, not the orchestrator). Then `grep -n 'SESSION_URL\|MODEL_NAME' <that file>` must print nothing. `<BRANCH>` and `<BASE>` stay: they are per-ticket and the brief supplies them.
...
2. **Pick.** Fill free slots with the highest-priority tickets whose files do not overlap ANY unmerged branch this loop has opened, running or awaiting merge (check the running briefs and `gh pr list --state open`); overlap with an unmerged PR manufactures the conflicts that shepherding then has to clean up. Apply the family-PR test above.
...
**When the agent cannot be reached** (session restarted, id gone, `SendMessage` errors): take over its worktree directly per `psy-dispatch`'s take-over flow, with one correction to the last mile. The PreToolUse hook keys on the SESSION cwd from its payload, so from a main-repo-anchored session the key is `<main repo>__<main repo's branch>` no matter which worktree you `cd` into, and the agent's existing marker never matches. The path that works: run the real fresh panel against `git -C <worktree> diff $(git -C <worktree> merge-base HEAD origin/main)...HEAD`, write the marker under your own session key on a passing verdict, open the PR with `gh pr create --head <branch>` as a standalone command, and disclose the take-over and the key substitution in the PR body. Or hand the `gh pr create` step to a fresh worktree-anchored agent. Never fabricate a marker without the panel. Redispatch a fresh agent only after the old worktree is confirmed dead and removed; never onto a branch a worktree still holds.
