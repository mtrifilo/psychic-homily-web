---
name: psy-dispatch
description: Dispatch a batch of Psychic Homily Linear tickets in parallel git worktrees. Use when the user provides a list of PSY-XXX tickets and asks to "tackle in parallel", "dispatch in worktrees", "batch these tickets", or otherwise work multiple tickets concurrently without blocking each other. One agent per ticket, each in its own isolated worktree, each runs `/code-review` before opening its PR. Resolves ambiguity via AskUserQuestion BEFORE dispatch.
...
9. **Verify base currency before AND after dispatch — and every agent rebases itself immediately before its PR opens (PSY-922).** Worktrees branch off local main; a stale base produces stale-fallout CI failures from unrelated work that landed during dispatch. Three layers: each agent branches off `origin/main` (not the worktree's inherited HEAD) at its own step 1, so the base is current at START; each agent runs per-agent template step 8.9 (fetch + rebase onto `origin/main` right before push) so its PR opens on a current merge-base regardless of how long its window ran; at step 7 (Stale-base recovery) the orchestrator re-checks and rebases anything that still drifted. The orchestrator layer is RECOVERY, not the mechanism — canonical: PR #908 (2026-05-30) opened 6 commits stale because only the before/after layers existed. See the stale-base anti-pattern below for the May 2026 dogfix-sweep example.
...
11. **Sub-agents a dispatched agent spawns are `subagent_type: "general-purpose"` with `model: "opus"`, never `fork`, at ANY spawn site** (review panels, research, anything). A fork inherits the agent's full orchestration prompt and can run the whole ship workflow on its own and return a fabricated completion report (PSY-1088 / PR #1246, `pattern_dispatch_fork_subagent_rogue.md`). Dispatched agents do have the Agent tool; the guard is on the spawn shape, not on spawning.
...
4. **If dirty (uncommitted changes) OR has unpushed commits:** two paths, ranked by safety:
   - **Default: STOP and surface to user.** Never auto-switch — risk of losing the user's in-flight context. Wait for explicit instruction before continuing.
   - **Alternative: branch-off-`origin/main` workaround.** When the user has explicitly said *"don't disturb my side branch"* OR previously approved this pattern in the current session, instruct each dispatched agent in their CRITICAL FIRST STEP prompt block to `git fetch origin && git checkout -b PSY-{N}/desc origin/main` — this overrides the harness-default HEAD-based branch and sidesteps the side-branch HEAD entirely. The user's working tree stays untouched; agents work cleanly off latest main. **Canonical: Wave 1 + Wave 2 of the 2026-05-24 session** — user had uncommitted PSY-823 then PSY-852 side-branch work across two dispatch waves; ALL 7 agents across both waves used this CRITICAL-FIRST-STEP pattern; zero PRs inherited the side branch's commits; zero conflicts with the user's parallel work. The prompt block looks like: *"Your worktree was created off the main repo's HEAD which is STALE (the user has a side branch checked out). Before any work, branch off the actual latest main: `git fetch origin && git checkout -b PSY-{N}/desc origin/main && git log -1 --oneline` (confirm you are now on the latest main)."*
...
**Any match is a HARD STOP on *unexamined* dispatch (PSY-922)** — a branch exists, a worktree holds it, or any PR in ANY state (open / merged / closed) matches the ticket number OR the title keywords, including under a *different* ticket number. Never branch-name-disambiguate around an existing PR, and never dispatch before triaging the match. Route by match type: a MERGED PR match means the work may already be shipped — see disposition 4 in **"Take-over flow when prior partial work exists"**; other matches route through dispositions 1–3 (disposition 2 is the only route back to a fresh dispatch, and only with explicit user consent to discard the prior work). Do NOT dispatch a fresh agent on a ticket whose branch is already held by another worktree — `git checkout -b` will fail at the agent's first step, and force-deleting the branch would silently destroy the parallel session's work.

**Stale-backlog check** (catches the *"ticket is already done by adjacent work that broadened scope"* false-positive — tickets that sat in Backlog while related work merged). For each ticket whose Linear `updated_at` is >1 day before dispatch, run a quick git-log check against the file paths the ticket touches BEFORE moving to In Progress:

```bash
git -C <main-repo> log --oneline -20 -- <relevant-paths-from-ticket>
```

If recent merges (last few days) include subject lines / PSY-IDs that look like they may have addressed this ticket's scope, re-read the ticket against current code BEFORE dispatching. **Canonical: PSY-782 (2026-05-24)** — filed before PSY-797 broadened scope to wrap `getShow` in `React.cache()`; would have been closed as "already done by PSY-797" pre-dispatch if the orchestrator had run `git log --oneline -- frontend/app/shows/` and seen the prior-art commit. Agent correctly caught it on the verify-first step (per `feedback_verify_ticket_hypothesis_first.md`) but the dispatch round-trip — agent spin-up + worktree create + read + report — was avoidable. Net cost of the check: ~5 seconds per stale ticket; net cost of the round-trip: ~2 minutes per stale ticket plus orchestrator cycles to close it cleanly.
...
When the per-ticket branch + worktree + PR cross-check (Step C) turns up a match for a ticket in this batch, you have a parallel-session race, a paused-mid-flight ticket, or already-shipped work. Four dispositions, ranked by value:

1. **Take over from the orchestrator (preferred when work is non-trivial).** Inspect the worktree state — `git -C <worktree-path> log main..HEAD --oneline` for committed work, `git -C <worktree-path> status` for uncommitted edits, `gh pr list --search "PSY-{N}"` for any opened PR. If the work is substantive (matches the ticket's AC partially or completely), the orchestrator handles the rest directly — verify the existing commits against AC, then run the FULL tail of the per-agent template, not a shortcut: steps 4–5 (tests + manual repro), `/code-review` (step 8) if not already done, the adversarial gate (below), the step-8.9 fetch + rebase onto `origin/main` (a taken-over worktree is stale-based BY CONSTRUCTION — its base predates the pause), push (`--force-with-lease` if the rebase rewrote pushed commits), and open the PR per step 10. For a PR that is already open (`gh pr edit --body` against an outdated convention), rebase before amending. **The adversarial gate in a take-over is NOT step 8.5's inline fallback** — that fallback is for an agent that genuinely lacks the Agent tool; in a take-over you spawn the real fresh-context panel (never substitute an inline read — that's the PSY-1206 anti-pattern) against the worktree's diff, extracted explicitly: `git -C <worktree> diff "$(git -C <worktree> merge-base HEAD origin/main)"...HEAD`. And know the marker mechanics: the `/adversarial-review` skill and its `gh pr create` hook both key on the ANCHORED cwd's repo+branch (no `-C`), not the worktree's — so from the main checkout the skill would review the wrong tree and write a marker for the wrong key, and a stale marker there can even satisfy the hook silently. After the panel passes against the worktree diff, follow psy-solo's hijack-recovery prescription: record the verdict for the REAL branch, run `gh pr create --head <branch>` with `ADVERSARIAL_REVIEW_SKIP=1`, and disclose the take-over + panel provenance in the PR body. DO NOT dispatch a fresh agent on top — they'd race with whoever was working in that worktree, the branch checkout would fail, and force-deleting would destroy the parallel session's work.
...
4. **Already shipped — possibly under another ticket (PSY-922).** A MERGED PR match (by ticket number OR by title keywords under a different number) means the work may already be on main. Read the merged PR's diff against this ticket's AC: if it satisfies the AC, do NOT dispatch — report it in the wave summary and close the ticket as superseded, citing the merged PR. If it partially satisfies, re-scope the ticket to the remainder before dispatching. Canonical: dup PR #909 (2026-05-30) was opened after PSY-918/#907 had already merged the identical skill-wiring work — the MERGED match was seen and not treated as a stop.
...
- **Verify PR body convention compliance.** For each PR opened, confirm the body contains the current convention's required sections:
   ```bash
   gh pr view <PR> --json body --jq .body | grep -cE "^## (Manual repro|Code review)"
   # Expected: 2 — one ## Manual repro header + one ## Code review header.
   ```
   If sections are missing — typically because a parallel session opened the PR off a stale skill snapshot, OR an orchestrator-takeover inherited a PR opened before a convention update merged — edit the body via `gh pr edit <PR> --body "<new content>"` to bring it up to spec. Reuse the agent's return-message details (Manual repro and Code review outcomes) verbatim where possible; the convention exists so reviewers can audit verification from the PR alone, not from the agent's chat trace. **Caught: May 2026 dogfix-2 (PR #563)** — parallel session opened PSY-613's PR before PR #559 (the convention-adding skill update) merged; orchestrator took over and edited the body to add the missing `## Manual repro` + `## Code review` sections. Without this check, drift accumulates: each merged off-spec PR sets a precedent that the next reviewer accepts.
...
- **Verify PR CI by reading check rows — never trust a bare `gh pr checks --watch` exit code.** `--watch` can exit 0 while a required check is still failing (observed 2026-06-08 on PR #1056: E2E Smoke red, `$?` was 0). After any watch, always re-run a fresh `gh pr checks <PR>` and read every row's conclusion before declaring green or surfacing "CI passed" to the user. Same discipline applies to `gh run watch` (see anti-pattern below for the `| tail` masking variant).

### 6.5. Cross-agent finding aggregation

Before moving to stale-base recovery, scan every returning agent's *Scope-adjacent observations* block (in their report) for findings that surface across multiple agents. Per `feedback_audit_recurring_bugs.md`: **at 2 instances of the same pattern across the wave, file a parallel audit ticket that sweeps the codebase**. Single-instance observations get noted in the wave summary; two-instance observations trigger an audit-ticket draft via `psy-ticket`.

Common cross-agent finding shapes:
- Same anti-pattern in different files / packages (e.g. "lowercase-leading user-visible error strings" surfaced by both PSY-587 + PSY-594 in different Go service files — May 10 wave)
- Same primitive-extraction opportunity in different feature areas (e.g. multiple agents notice an inline-banner shape worth extracting)
- Same convention drift (e.g. multiple agents notice their target file uses a deprecated helper)

What to file:
- **Audit ticket** for the discovery sweep (`confidence:75` if a clear remediation plan exists; `confidence:25` if the sweep needs a spike first to decide remediation). Title: `"Audit: <pattern> across <scope>"`. Description quotes the 2+ agent reports verbatim with PSY-NN attribution.
- Do NOT widen any in-flight PR's scope to address the finding. The PRs ship their stated scope; the audit ticket captures the systemic concern.

This step is cheap (one read per agent report) and prevents the same finding from being re-discovered by the next 3 agents. **May 10 wave** is the canonical example: PSY-587 + PSY-594 both flagged lowercase-leading error strings in their respective Go service files; orchestrator surfaced for follow-up at wave summary time.
...
gh pr checks <PR>   # if origin/main moved, check CI on each PR — read the rows; never gate on a prior `gh pr checks --watch` exit code alone (see step 6)
```
...
4. Append, OR replace if the new entry resolves an existing caveat (e.g. PSY-612 dropped the "canonical chain is NOT project-wide" caveat from the PSY-353 entry). **Treat the agent's proposed text as a draft, not a verbatim insert** — trim and restructure to match the existing entries' style (one-line **bold key** at start, parenthetical PSY-XXX references, **CAVEAT** marker for known gaps, in-line code spans for file paths and identifiers). The agent doesn't see the surrounding entries; the orchestrator does, and consistent style makes the index scannable. Keep any one-line index pointer in `MEMORY.md`'s top-level index under ~150 chars per the existing memory rules.
...
- **Chunk long sweeps and verify the count reaches the target (PSY-922).** A sweep of tens/hundreds of `git worktree remove` calls run as one long foreground loop gets silently backgrounded by the harness mid-operation, fragmenting it across turns (canonical 2026-05-30: a ~198-worktree sweep). Record the baseline BEFORE starting — `git worktree list | wc -l` (note it includes the main checkout, so expected survivors = baseline − removals, main included in both). Run removals in chunks of ~10–15 per command, and end the sweep by re-running the count and comparing against that expectation — never infer completion from the loop having been started.
...
- **`--mode=isolated`** — full per-worktree stack:
  ...
  - Also sets `NEXT_PUBLIC_OAUTH_BACKEND_URL=$STACK_BACKEND_URL` (PSY-1649). OAuth is a full-page redirect to the backend's `/auth/login/google` and does NOT survive the `/api` proxy, so it needs the backend's own origin. That is the one requirement `NEXT_PUBLIC_API_URL` cannot also serve. Unset, it falls back to `NEXT_PUBLIC_API_URL`, so deployed environments are unaffected.
...
- Sub-agents you spawn, for ANY purpose, are `subagent_type: "general-purpose"` with `model: "opus"`; NEVER `subagent_type: "fork"` (a fork inherits this whole prompt and can commit, push, and open the PR on its own).
...
# Work plan
1. **Verify isolation FIRST, then re-base on actual latest main.** Run `git rev-parse --show-toplevel` — it must resolve under `.claude/worktrees/`, not the main repo root. Then, because the worktree was cut from the main repo's *current HEAD* — which a parallel session may have left on a side branch or be actively ping-ponging (the shared checkout is contested in this repo) — do NOT trust that HEAD: `git fetch origin main && git checkout -B PSY-{N}/{kebab-desc} origin/main && git log -1 --oneline` (confirm you're on latest main). This is the CRITICAL FIRST STEP; skipping it inherits stray commits from whatever branch the main checkout happened to be on into your PR.
2. **Verify the premise BEFORE implementing.** Explore: {what to read first}. If the ticket's *"Open questions / spike needs"* section says to "verify first" OR the ticket sat in Backlog >1 day before dispatch, run `git log --oneline -20 -- <relevant-paths>` looking for recent commits whose subject suggests scope overlap with this ticket. If you find the work already done, STOP and report back with the prior-art commit + PR citation — DO NOT open a no-op PR. **Canonical: PSY-782 (2026-05-24)** — agent correctly STOPPED after finding `React.cache()` already wrapped `getShow` (shipped by PSY-797 / PR #796); no PR opened; orchestrator closed the ticket as superseded with a comment citing the prior-art commit. Per `feedback_verify_ticket_hypothesis_first.md`: bug-fix tickets name a hypothesis that may be wrong; verify before spending dispatch cycles on a wrong-premise implementation.
3. Implement the fix.
4. **Run all relevant local tests. Failure blocks push.** This is non-negotiable. Run, in order of how directly they exercise your diff:
   ...
   - **`go test ./...` + `go generate ./...` are MANDATORY (not just "if large") when the diff adds or changes a migration, table, or interface — a scoped `go test ./<pkg>` MISSES cross-package CI gates.** Two required CI checks live OUTSIDE the package you edit: **Mocks Generator Drift** (`internal/api/handlers/shared/testhelpers/mocks_gen.go`, regenerated by `go generate ./...` — a new/changed interface method silently drifts it) and the **user-FK fixtures-allowlist** contract (`internal/api/handlers/admin` — a new table carrying a `users` FK must be classified there). A scoped `go test` passes green locally while both fail CI. **2026-06-22/24:** PSY-1199 (#1244) + PSY-855 (#1245) BOTH merged red on exactly these → main red → CI-restoration PR #1250. So any diff adding a migration/table/interface MUST run `go generate ./...` (and commit any regenerated `mocks_gen.go`) AND `go test ./...` before push, regardless of diff size.
   - **Backend lint (a CI gate — `go vet` / `go test` do NOT catch it):** `cd backend && go run honnef.co/go/tools/cmd/staticcheck@2025.1.1 ./<package(s) you touched>/...` and fix every finding in your files. CI's required **Backend Lint** runs golangci-lint/staticcheck. **PSY-1119 (2026-06-16)** passed `go build` + `go test` + `go vet` locally but failed CI Backend Lint on a `QF1012` (`fmt.Fprintf(&b, …)` over `b.WriteString(fmt.Sprintf(…))`), forcing an orchestrator lint-fix commit. Run it whenever your diff touches `backend/`, and list it in the PR Test-plan.
   - **Frontend type safety:** `cd frontend && bun run typecheck`.
   - **API types drift (a CI gate — fires on BACKEND changes, so it is easy to miss on a backend-only PR):** if the diff touches the API surface, run `cd frontend && bun run api:types` and COMMIT the regenerated `frontend/types/api.d.ts` in the SAME PR. `frontend/types/api.d.ts` is a committed artifact generated from the OpenAPI spec (PSY-1550); CI's **API Types Drift** job regenerates and fails on any diff. "API surface" is broader than adding a route: it includes changing a request/response struct or a `json:` tag, and **moving an operation between Huma API instances** (sub-API → main API changes the PUBLISHED document even though no handler changed). Two PRs can each be green alone and red merged, so this must be done per-PR, not swept up later. Caught twice on day one — PSY-1577's scene-week endpoints turned main red, and the PSY-1598 spike hit it moving a report route. See `pattern_frontend_api_type_drift.md`.
   - **Frontend lint (a CI gate — easy to forget):** `cd frontend && bun run lint`. CI's required **Frontend Lint** check runs eslint, and typecheck + unit tests do NOT catch lint errors (e.g. `react/display-name` on an anonymous test mock). **PSY-992 (PR #1004, 2026-06-06)** passed typecheck + tests locally but failed CI Frontend Lint on exactly that. Run `bun run lint` whenever your diff touches `frontend/`, and list it in the PR Test-plan.
   ...
   - **E2E:** run the spec(s) covering any page/surface your diff changes — **not just specs you *edit***. **If your change alters a form, flow, or route an E2E spec exercises** (signup, login, password recovery, a key journey), run that spec **even when the diff touches no `frontend/e2e/` files** — the "only run E2E when `e2e/` changed" rule is a **floor, not a ceiling**. **Canonical: PSY-1023 (2026-06-08)** — a required age-confirmation checkbox on signup never triggered `e2e/auth/register.spec.ts` (the new field gates `canSubmit`); the spec failed post-merge and broke main's E2E. PR-CI runs E2E `@smoke` only, so a diff that rewrites a page can break that page's *non-`@smoke`* spec invisibly until full-E2E-on-main (PSY-389 rewrote `/` → `home.spec`; PSY-1016 rewrote `/radio` → `radio.spec`; both shipped main-reds, 2026-06-08). Changing the auth/nav surface? mind `follow-and-attendance.spec`'s `getByRole('button', { name: 'User menu' })`. Run: `cd frontend && bun run test:e2e -- <path-to-spec>` whenever you touched `frontend/e2e/`, rewrote a spec-covered page, OR altered a form/flow/route a spec drives. The E2E global-setup hard-requires port 8080 to be free; if the user's dev backend occupies 8080, STOP and report back so the orchestrator can ask the user to free it. Do NOT skip the E2E run silently.
...
5. **Manual repro the change end-to-end.** Tests verify code-correct; manual repro verifies feature-correct (per CLAUDE.md: *"Type checking and test suites verify code correctness, not feature correctness."*). Use the `scripts/dispatch/stack-up.sh` helper for stack management (see the "Per-worktree dev stack" section above for the full mode model).
   ...
   - **Exercise the change:**
     ...
     - `mode=shared` / `mode=isolated`: navigate `$STACK_FRONTEND_URL`, exercise the canonical failing path the ticket described, and capture a screenshot of the new behaviour into `dogfood-output/PSY-{N}/screenshots/<short-name>.png`. STOP if the canonical failure mode does NOT now surface in the UI — the fix is incomplete; iterate from step 3 before proceeding. **Browser choice — parallel-wave frontend agents MUST use `agent-browser` (its own isolated Chrome), NOT the shared `chrome-devtools` MCP.** When multiple frontend agents run concurrently, the shared `chrome-devtools` MCP browser's page-selection gets hijacked across sessions — the `[selected]` page jumps to a sibling agent's dev port mid-navigation, producing contaminated/corrupted screenshots and forcing retries (see the browser-layer collision anti-pattern below). `agent-browser` gives each agent its own isolated context, so concurrent navigation + screenshot capture stay clean. A single non-parallel dispatch may use either.
   - **Upload screenshots for the PR — gitignored paths never reach reviewers, and DRAFT releases 404.** `dogfood-output/` is gitignored, so a path reference in the PR body is unreviewable text, and the files vanish when the worktree is cleaned up. Host the PNGs on a **PUBLISHED** GitHub release (NOT `--draft`) and embed the **`releases/download/<tag>/<file>`** asset URLs in the PR body's `## Screenshots` section:
     ```bash
     # Create PUBLISHED (no --draft). A draft is parked under an `untagged-<hash>` ref
     # whose releases/download/untagged-…/ URLs 404 in the PR (broken images).
     gh release create psy-{N}-screenshots --title "PSY-{N} screenshots" \
       --notes "Screenshot assets for PSY-{N} PR." \
       dogfood-output/PSY-{N}/screenshots/*.png
     # Embed THIS URL form, one per file (do NOT use `.assets[].url` — that's the API
     # URL and also won't render):
     #   https://github.com/<owner>/<repo>/releases/download/psy-{N}-screenshots/<file>.png
     ```
     After capturing, verify each PNG shows what its filename claims (Read it back) AND confirm the embedded URL resolves (`curl -sI <url>` → `HTTP … 302`). Do NOT delete the release after the PR opens — the embeds depend on it persisting. **Caught 2026-06-06 (Production Ready Audit batch):** 3 PRs (PSY-980/983/992) embedded `untagged-<hash>` draft-release URLs that 404'd; the orchestrator had to publish each release + rewrite each PR body to `releases/download/<tag>/<file>`. (Earlier 2026-05-31 wave: PRs referenced gitignored paths only, requiring re-capture.)
   ...
   - **Stall-takeover carveout (PSY-627 watchdog pattern at chrome-devtools MCP).** When a dispatched agent stalls at step 5's manual repro — most commonly during browser navigation, login, or screenshot capture via chrome-devtools MCP — and has ALREADY committed implementation + tests + `/code-review` before the stall, the orchestrator's takeover MAY apply the render-only carveout to ship the PR without re-running browser verification. Gates: (1) unit tests at the changed surface must comprehensively cover the rendered output / form-submit shape / new behaviour (NOT just exist — review them); (2) the PR body's Manual repro section MUST explicitly disclose the stall, list the test coverage that substitutes for browser-MCP, and recommend a pre-merge Vercel-preview spot-check; (3) the diff must be render-or-data-refactor-shaped — new user-facing behaviour with no test coverage does NOT qualify (re-dispatch with the takeover-context prompt instead). **Canonical examples: May 10 wave** — PSY-585 (Create modal field) + PSY-599 (URL pre-validate) + PSY-630 (banner sweep) + PSY-574 (primitive migration) + PSY-575 (banner primitive) ALL hit the screenshot-capture stall and shipped via this carveout with disclosure. **Orchestrator recovery procedure when this happens**: (i) `git -C <worktree> log main..HEAD --oneline` to confirm commits present; (ii) `git -C <worktree> reset --hard origin/<branch>` ONLY if local diverged from a prior failed-rebase run (origin is the authoritative pushed state); (iii) `git -C <worktree> rebase origin/main` — resolve any conflicts with merged tickets that overlap scope (e.g. PSY-599 + PSY-563's drawer migration on `EntityEditDrawer.tsx`); (iv) `git diff origin/main --stat` to verify diff scope post-rebase matches the ticket's intended files (stale-base noise looks like the diff reverting recently-merged work — that's a rebase you forgot to do, not real intent); (v) run typecheck + relevant test scopes directly; (vi) `git push --force-with-lease`; (vii) `gh pr create` with the carveout disclosure in the PR body. Never silently merge a takeover PR without the disclosure — the reviewer needs to know the unit tests + Vercel preview are the only pre-merge signal.
   - **Post-`/code-review` push-step stalls also qualify under this carveout.** **PSY-842 (2026-05-24)** shipped via this pattern: agent committed implementation, ran `/code-review` cleanly (0 findings), then never reached `git push` / `gh pr create` — the task-completion notification returned only the truncated `/code-review` JSON output, no standard structured report. Orchestrator re-verified tests locally per `feedback_local_test_before_push.md` (`bun run test:run lib/api` + `bun run typecheck`, both clean against the agent's committed branch), pushed, and opened the PR with disclosure: *"Agent stalled after committing + running /code-review; orchestrator took over the push + PR step per psy-dispatch's stall-takeover carveout. Tests re-verified locally before push."* Same gates as the chrome-devtools-MCP version apply: commits must be present, tests must be green when re-run by orchestrator, code-review must have run, PR body MUST disclose the takeover so reviewers know the agent's final-state report was orchestrator-augmented. Signal that this kind of stall happened: the task-completion notification text is suspiciously short (just the tail of the last operation) and the worktree has commits but no `git push` evidence (`gh pr list --search "PSY-{N}"` returns empty).
   - **Before ANY stall-takeover, CONFIRM THE AGENT IS DEAD — a missing completion notification is NOT proof of a stall, and the takeover carveouts above presuppose a genuinely-dead agent.** A live agent with committed work + a clean worktree + no notification yet looks IDENTICAL to a stalled one — critically, it is ALSO exactly what an agent mid-`/adversarial-review` looks like (that gate runs long and silent before the agent re-pushes its fix commit + rewrites the PR body). Taking over here races a still-working agent and can clobber its in-flight fixes. **2026-06-22 (PSY-1206):** the orchestrator judged a LIVE agent "stalled" on that weak evidence, ran a SHALLOW inline adversarial pass, called it CLEAN (it wasn't), and used `ADVERSARIAL_REVIEW_SKIP` to open a PR on a false gate result; the agent then self-corrected — re-pushing its real adversarial-review fixes and rewriting the body. Gate the takeover: (1) positively confirm death — the process is actually gone AND no work has landed recently (`git -C <worktree> log -1 --format=%cr` shows the last commit's age; a commit from seconds/a-minute ago = still working, do NOT take over), not merely "no notification arrived"; (2) NEVER substitute a quick inline adversarial read for the real `/adversarial-review` gate — if you cannot run the real gate, do NOT `ADVERSARIAL_REVIEW_SKIP` a "looks clean to me" verdict. When uncertain, wait or re-dispatch with takeover context rather than race the agent.
...
8. Run `/simplify` (Skill tool, skill: "simplify"; `/code-review` is user-invocable only and errors for an agent, see `pattern_code_review_not_agent_invocable.md`; say in the PR body that `/simplify` ran in its place). If it edited files, commit them as a SEPARATE commit `PSY-{N}: code-review pass`. **Re-run the relevant local tests from step 4** if code-review changed anything substantive. Re-run the manual repro from step 5 only if code-review edited a file you exercised in step 5.
8.5. Run `/adversarial-review` (Skill tool, skill: "adversarial-review") — the hostile pre-PR gate; the change must EARN a clean verdict, not get the benefit of the doubt. **Check your own tool list first: a dispatched worktree agent usually DOES have the Agent tool** (verified 2026-09-06, PSY-2026: the agent spawned parallel fresh-context panels across three rounds), so spawn the panel as fresh `subagent_type: "general-purpose"` sub-agents with `model: "opus"`, all in one message; NEVER `subagent_type: "fork"` — a fork inherits your full orchestration prompt and can run the whole ship workflow on its own (PSY-1088 / PR #1246, `pattern_dispatch_fork_subagent_rogue.md`). Only if you genuinely lack the Agent tool, or the spawn fails and one retry fails too, tell the orchestrator you are falling back (an inline pass runs long and silent and trips the stall watchdog) and then run the four lenses INLINE (Saboteur / Future-Maintainer / Security / Completeness) against your branch diff, applying the skill's finding bar, and say in the PR body that no independent context reviewed the diff (the same fallback shape `/code-review` uses from a worktree; `pattern_subagent_inline_code_review.md` predates the 2026-09-06 observation and its "no Agent tool" premise is superseded; `pattern_dispatch_fork_subagent_rogue.md`'s "forbid all sub-agents" rule is narrowed to forks by rule 11 above). Inline is expected in that case, not an error. The Panel line names ONLY the lenses that actually reported; a lens that stalled or returned nothing is not listed and is never corroboration. Verdict handling:
...
8.9. **Rebase onto latest `origin/main` — mandatory, immediately before push (PSY-922, non-negotiable #9).** One chained invocation, measured AND echoed before the rebase (afterwards the range is always empty, and a conflicting rebase exits non-zero — the echo must come first or a conflict eats the count): `git fetch origin main && PULLED=$(git rev-list --count HEAD..origin/main) && echo "pulled in: $PULLED" && git rebase origin/main`. Your worktree branched off main at DISPATCH time; siblings and parallel sessions have merged since. If `PULLED` > 0, re-run the step-4 gates that exercise your diff (typecheck + scoped tests at minimum) BEFORE pushing — see the interaction failure locally, not in CI — and if the rebase changed files your step-8.5 adversarial pass covered, re-run the lenses. A rewriting rebase also means any commit SHAs already written into your PR-body draft are dangling — re-derive them from `git log --oneline origin/main..HEAD`. Conflicts: resolve only what is mechanically obvious within your ticket's scope; otherwise STOP and report (never auto-resolve a semantic conflict with a sibling's merged work).
9. **Push branch with `-u origin <branch>` — or `--force-with-lease` if step 8.9's rebase rewrote commits you had already pushed (never bare `--force`, which clobbers a parallel push; `--force-with-lease` bails if the remote moved). If your diff adds a migration, re-stamp its filename at PUSH TIME.** A migration filename's timestamp has to beat every migration already merged on `origin/main` when your PR *lands*, not when you *authored* it. golang-migrate silently skips any migration at or below the current `schema_migrations` pointer (it reports "no change" and exits 0), so a backdated file never applies and its columns simply never exist. Immediately before pushing, and again after any rebase that pulls in a sibling's merged migration, run `git fetch origin main && bash scripts/check_migration_timestamps.sh origin/main`. If it fails, `git mv` BOTH the `.up.sql` and the `.down.sql` to a fresh `date -u +%Y%m%d%H%M%S` stamp with contents unchanged, then amend or add a commit. **Canonical: stage outage 2026-08-02** (PSY-1708): two parallel worktrees authored `20260801120000` and `20260801143000` in author-time order, merged in the reverse order, and the lower-stamped migration was skipped in silence; every stage deploy failed for four hours until the DDL was replayed by hand. The Migration Lint CI job now fails this, but catching it locally saves a red-CI round trip.
10. Open PR with `gh pr create`. **Body-file freshness + real IDs (PSY-922):** `Read` the body file in the same turn that consumes it via `--body-file`, and never batch the refreshing `Write` with the consuming command (a cancelled `Write` otherwise ships a stale body silently — canonical 2026-05-30). Every PSY/PR number in the body must come from a just-run `linear issue view` / `gh pr view` / creation output, never from incrementing an adjacent number. Body template:
    ```
    ## Summary
    - <bullet 1>
    - <bullet 2>
    ...
    Panel: <lens names that reported> — <fresh sub-agents, no prior session context | run INLINE: no independent context reviewed this diff>.

    Closes PSY-{N}
    ```
    The Test plan section must list the actual commands you ran in step 4, with `[x]` checkboxes (not unchecked) — they're statements of "I verified this", not aspirations. The Manual repro section is the artifact from step 5; without it the PR is unverified and the orchestrator escalates as a process violation. The Code review section makes the code-review outcome auditable from the PR alone, not just the agent's return-message.
...
- `/code-review` diff (or "no changes"). If code-review changed code, list the post-code-review re-run of the test commands from step 4. Manual repro re-run only if code-review edited a file you exercised in step 5.
...
- **Skipping `/code-review` for "small" tickets.** The discipline is the point. Most small tickets produce no code-review diff anyway; running it costs nothing.
...
- **Dispatching from a stale local main (whole-batch CI failure).** Worktrees branch off local main; if it's behind origin/main at dispatch time, every PR inherits the same stale base. **The May 2026 dogfix sweep (PSY-558/559/560/561/562)** hit this: local main was 8 commits behind origin/main; two of those commits (PSY-357 + PSY-359) added test files exercising new collection paths; ALL 5 PRs failed the same Backend + E2E suites despite each PR's diff being clean and unrelated to collections. Frontend unit tests passed on every PR — the only suite actually exercising the diff. The signature is **identical CI failure shape across PRs that touch different files**. Three catches, in order: per-agent step 1 (branch off `origin/main` after an explicit fetch) makes the base current at start; per-agent step 8.9 (fetch + rebase immediately before push, PSY-922) catches the moved-during-the-agent's-window case at the source; step 7 is the orchestrator's recovery for anything that still drifted. Recovery is a parallel `git rebase origin/main && git push --force-with-lease origin <branch>` per worktree (per step 7).
...
- **Calling `gh pr create` from the main repo's CWD while on `main`, without `--head <branch>`.** `gh` defaults `--head` to the current HEAD branch; if that's `main` (because you committed to a feature branch then switched back to `main` before pushing/PR-creating, per the side-branch checkout-recovery flow in Step A), the call errors with `"head branch 'main' is the same as base branch 'main', cannot create a pull request"`. **Always pass `--head <branch-name>` explicitly** when opening a PR for a branch you've already pushed but switched away from. Caught: May 2026 dogfix-2 — orchestrator opened the skill-update PR after switching back to main, hit the error, retried with `--head` (one wasted turn). Mechanical; one extra flag.
...
- **Stalls cluster at chrome-devtools MCP screenshot / login / navigation calls (PSY-627 watchdog).** May 10 wave hit this **5 of 8 agents** (PSY-585/599/630/574/575) — all at step 5 manual repro after completing implementation + tests + `/code-review`. PSY-627's general "agents stall on laptop sleep / WiFi" hypothesis is real, but the empirical concentration is on browser-MCP calls (network-bound, long-running, no intermediate stream output → the 600s watchdog kills them before recovery). The work itself is fine — the wasted cost is the orchestrator-takeover for what should have been the last 2 steps of the per-agent template. Mitigation is the stall-takeover carveout in step 5 of the per-agent template above: rebase, verify diff scope, run tests directly, push, open PR with disclosure. Do NOT silently merge takeover PRs without the carveout disclosure — the manual repro wasn't done, the reviewer needs to know. Pattern observation worth surfacing to the user at wave summary time so PSY-627's investigation can prioritise the MCP-browser path specifically; aggregate stall stats (how many agents, what tool, what step) when you see them.
- **Shared `chrome-devtools` MCP browser gets hijacked across parallel sessions (browser-layer collision).** This is the dispatch parallel-session collision (see "The ironclad rules" cross-cutting Step C + the `gh pr list --search` per-ticket cross-check) manifesting at the BROWSER layer rather than the branch / PR layer. **Caught 2026-06-06/07 (tag-facet wave, 5 concurrent frontend agents):** 4 of 5 agents independently reported the shared `chrome-devtools` MCP browser's `[selected]` page kept getting hijacked by sibling agents — page-selection jumped to another agent's dev port mid-flight, yielding contaminated / corrupted screenshots and forcing retries. Each agent worked around it by falling back to `agent-browser`'s isolated Chrome. Fix encoded in step 5 of the per-agent template: parallel-wave frontend agents use `agent-browser` (own isolated context) for navigation + screenshot capture; the shared MCP browser is reserved for single, non-parallel dispatches. Distinct from the chrome-devtools-MCP **stall** entry above (that's the 600s watchdog killing a long-running browser call); this one is cross-session page-selection contention that produces wrong-but-not-hung output.
- **Scope deviation must be documented in the PR body, not hidden.** If an agent (or orchestrator-takeover) ships less than the ticket's full stated scope — N of M sites migrated, partial cluster addressed, intentional skip — the PR body MUST have an explicit `## Scope deviation` section explaining WHAT was skipped, WHY, and what follow-up is recommended. **Canonical example: PSY-574 (May 10)** — VenueCard intentionally skipped from the 7-card migration because its nested-Link structure was incompatible with the new primitive AND VenueCard has a second `<Link>` for show-details that conflicts with the outer-Link invariant. The PR body documented "6/7 with rationale" so reviewers see the deviation without reading the agent's chat trace. Hiding scope deviations defeats the one-ticket-one-PR contract: a future reader assumes the PR shipped everything the ticket asked for. Encoded in the per-agent template's step 10 PR body convention — when scope deviates, add the section.
- **Cherry-pick-to-unblock pattern (carveout, NOT an anti-pattern).** When a separate PR's fix is blocking another PR's CI — and waiting for the separate PR to merge + then rebasing the blocked PR would burn another full CI cycle + risk a stale-base round — cherry-picking the fix commit directly into the blocked PR's branch is the pragmatic unblock. Gates:
   1. The fix must be small + obviously correct (no behaviour change beyond what the source ticket described).
   2. The blocked PR's body MUST disclose the cherry-pick + cite the source PR (e.g. `"## Why two commits in one PR — PSY-851 cherry-picked to unblock CI"`).
   3. The source PR is closed as `"superseded by cherry-pick in PR #X"` with a comment so the source ticket's history is preserved.
...
- **Trusting CI watch exit codes without re-reading check rows.** Two distinct failure modes: (1) **`gh pr checks <#> --watch` can exit 0 while checks are still failing** — **Caught 2026-06-08 (PSY-1042 / PR #1056):** watch returned exit 0 while E2E Smoke was red. Always re-run a fresh `gh pr checks <#>` and read every row before declaring green. (2) **Piping `gh run watch --exit-status` through `tail`/`grep` swallows the failure exit code** — a pipeline's exit code is the LAST command's (tail/grep = 0), so a failed CI run reports as success. **Caught 2026-05-31 (PSY-945):** orchestrator's background watch `gh run watch <id> --exit-status | tail -3` exited 0 on a run that had FAILED; the wrong signal survived until a direct `gh pr view --json statusCheckRollup` contradicted it, costing one diagnosis round. Correct shape for run-watch: redirect instead of pipe, then echo the code on its own line — `gh run watch <id> --exit-status > /tmp/watch.log 2>&1; echo "WATCH_EXIT_CODE=$?"` — and read the code from the output, never from the task's pipeline exit. For pr-checks: skip `--watch` for gating entirely; poll with fresh `gh pr checks <#>`. Related zsh gotcha from PSY-945: `status` is a READ-ONLY zsh special variable; `status=$?` errors out and kills the surrounding script — name it anything else.
