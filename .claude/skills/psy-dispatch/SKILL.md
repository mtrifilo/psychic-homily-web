...
9. **Verify base currency before AND after dispatch — and every agent rebases itself immediately before its PR opens (PSY-922).** Worktrees branch off local main; a stale base produces stale-fallout CI failures from unrelated work that landed during dispatch. Three layers: pre-flight (before step 1) syncs local main with origin/main; each agent runs per-agent template step 8.9 (fetch + rebase onto `origin/main` right before push) so its PR opens on a current merge-base regardless of how long its window ran; after step 6 the orchestrator re-checks and rebases anything that still drifted. The orchestrator layer is RECOVERY, not the mechanism — canonical: PR #908 (2026-05-30) opened 6 commits stale because only the before/after layers existed. See the stale-base anti-pattern below for the May 2026 dogfix-sweep example.
...
1. **Take over from the orchestrator (preferred when work is non-trivial).** Inspect the worktree state — `git -C <worktree-path> log main..HEAD --oneline` for committed work, `git -C <worktree-path> status` for uncommitted edits, `gh pr list --search "PSY-{N}"` for any opened PR. If the work is substantive (matches the ticket's AC partially or completely), the orchestrator handles the rest directly — verify the existing commits against AC, run typecheck + tests + manual repro per the per-agent template's steps 4–5, run `/code-review` if not already done, push and open the PR (or `gh pr edit --body` if a PR is already open against an outdated convention). DO NOT dispatch a fresh agent on top — they'd race with whoever was working in that worktree, the branch checkout would fail, and force-deleting would destroy the parallel session's work.
...
- **Chunk long sweeps and verify the count reaches the target (PSY-922 defect 7).** A sweep of tens/hundreds of `git worktree remove` calls run as one long foreground loop gets silently backgrounded by the harness mid-operation, fragmenting it across turns (canonical 2026-05-30: a ~198-worktree sweep). Run removals in chunks of ~10–15 per command, and end the sweep with an explicit verification (`git worktree list | wc -l` matches the expected survivor count) before declaring it done — never infer completion from the loop having been started.
