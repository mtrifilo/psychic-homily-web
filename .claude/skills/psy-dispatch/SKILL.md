...
10. Open PR with `gh pr create`. **Body-file freshness + real IDs (PSY-922):** `Read` the body file in the same turn that consumes it via `--body-file`, and never batch the refreshing `Write` with the consuming command (a cancelled `Write` otherwise ships a stale body silently — canonical 2026-05-30). Every PSY/PR number in the body must come from a just-run `linear issue view` / `gh pr view` / creation output, never from incrementing an adjacent number. Body template:
    ...
    Panel: Saboteur · Future-Maintainer · Security · Completeness — <fresh sub-agents, no prior session context | run INLINE: no independent context reviewed this diff>.
