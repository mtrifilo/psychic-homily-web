...
4. **Run all relevant local tests. Failure blocks push.** This is non-negotiable. Run, in order of how directly they exercise your diff:
   ...
   - **E2E:** run the spec(s) covering any page/surface your diff changes — **not just specs you *edit***. PR-CI runs E2E `@smoke` only, so a diff that rewrites a page can break that page's *non-`@smoke`* spec invisibly until full-E2E-on-main (PSY-389 rewrote `/` → `home.spec`; PSY-1016 rewrote `/radio` → `radio.spec`; both shipped main-reds, 2026-06-08). Changing the auth/nav surface? mind `follow-and-attendance.spec`'s `getByRole('button', { name: 'User menu' })`. If you modified any file under `frontend/e2e/` OR rewrote a spec-covered page: `cd frontend && bun run test:e2e -- <path-to-spec>`. The E2E global-setup hard-requires port 8080 to be free; if the user's dev backend occupies 8080, STOP and report back so the orchestrator can ask the user to free it. Do NOT skip the E2E run silently.
