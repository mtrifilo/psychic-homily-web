...
10. Open PR with `gh pr create`. Body template:
    ...
    Closes PSY-{N}
    ```
    The Test plan section must list the actual commands you ran in step 4, with `[x]` checkboxes (not unchecked) — they're statements of "I verified this", not aspirations. The Manual repro section is the artifact from step 5; without it the PR is unverified and the orchestrator escalates as a process violation. The Simplify section makes the code-review outcome auditable from the PR alone, not just the agent's return-message.
...
- `feedback_code-review_before_pr.md` — `/code-review` AND relevant local tests run before every PR (single-ticket or batched); failure blocks push, escalate to orchestrator instead of pushing past it.
