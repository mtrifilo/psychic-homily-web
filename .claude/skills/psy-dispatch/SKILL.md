...
**Any match is a HARD STOP on fresh dispatch (PSY-922)** — a branch exists, a worktree holds it, or any PR in ANY state (open / merged / closed) matches the ticket number OR the title keywords, including under a *different* ticket number. Never branch-name-disambiguate around an existing PR. Route by match type: a MERGED PR match means the work may already be shipped — see disposition 4 in **"Take-over flow when prior partial work exists"**; other matches route through dispositions 1–3. Do NOT dispatch a fresh agent on a ticket whose branch is already held by another worktree — `git checkout -b` will fail at the agent's first step, and force-deleting the branch would silently destroy the parallel session's work.
...
1. **Take over from the orchestrator (preferred when work is non-trivial).** Inspect the worktree state — `git -C <worktree-path> log main..HEAD --oneline` for committed work, `git -C <worktree-path> status` for uncommitted edits, `gh pr list --search "PSY-{N}"` for any opened PR. If the work is substantive (matches the ticket's AC partially or completely), the orchestrator handles the rest directly — verify the existing commits against AC, then run the FULL tail of the per-agent template, not a shortcut: steps 4–5 (tests + manual repro), `/code-review` (step 8) if not already done, `/adversarial-review` (step 8.5 — the `gh pr create` hook will hard-block without its pass-marker), the step-8.9 fetch + rebase onto `origin/main` (a taken-over worktree is stale-based BY CONSTRUCTION — its base predates the pause), push (`--force-with-lease` if the rebase rewrote pushed commits), and open the PR per step 10. For a PR that is already open (`gh pr edit --body` against an outdated convention), rebase before amending. DO NOT dispatch a fresh agent on top — they'd race with whoever was working in that worktree, the branch checkout would fail, and force-deleting would destroy the parallel session's work.
...
8.9. **Rebase onto latest `origin/main` — mandatory, immediately before push (PSY-922, non-negotiable #9).** Measure BEFORE rebasing (afterwards the range is always empty): `git fetch origin main && PULLED=$(git rev-list --count HEAD..origin/main) && git rebase origin/main && echo "pulled in: $PULLED"`. Your worktree branched off main at DISPATCH time; siblings and parallel sessions have merged since. If `PULLED` > 0, re-run the step-4 gates that exercise your diff (typecheck + scoped tests at minimum) BEFORE pushing — see the interaction failure locally, not in CI — and if the rebase changed files your step-8.5 adversarial pass covered, re-run the lenses. A rewriting rebase also means any commit SHAs already written into your PR-body draft are dangling — re-derive them from `git log --oneline origin/main..HEAD`. Conflicts: resolve only what is mechanically obvious within your ticket's scope; otherwise STOP and report (never auto-resolve a semantic conflict with a sibling's merged work).
