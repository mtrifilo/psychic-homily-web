...
5. **Dry-run + the artist-skip QA scan (MANDATORY for label rosters — collision rate is high)** — `bun run src/entry.ts --env <env> batch /tmp/ph-ingest.json`. Map every SKIP back to the proposed name (the skip header shows the *existing* matched entity): the 0.6 fuzzy threshold conflates distinct artists, and short/common punk band names collide heavily — **Feel It hit 4 false matches in 7 skips (Fan Club→"Yot Club", It Thing→"Nothing", Spllit→"Split", Vacation→"Medication")**, on top of Sacred Bones' (**Institute→"Prostitute", Lathe of Heaven→"Left of Heaven", Cheena→"Cheem"**, plus a collab→member, **Emma Ruth Rundle & Thou→"Emma Ruth Rundle"**). A wrong skip also links the *wrong* existing artist to the label. **Fix:** pre-create each distinct artist via `POST /admin/artists` (exact find-or-create) so its 1.0 match wins, then re-run — and **pre-create it WITH its `bandcamp`/social fields, not bare** (`POST /admin/artists {"name":…, "bandcamp":…}`), because the batch can't enrich an *existing* artist's social fields (see *Known CLI limitation* below), so a name-only pre-create strands the band without its Bandcamp. For a **real** duplicate that's missing its Bandcamp, `PATCH /admin/artists/{id} {"bandcamp":…}` directly. Do NOT pre-create via `ph submit artist` — that fuzzy-matches too.
...
> **Known CLI limitation (tracked: PSY-1170) — the batch cannot enrich an *existing* artist's social links.** Two field-name mismatches in `cli/src/lib/duplicates.ts` break it: the dedup comparison (`ARTIST_FIELDS`) reads `bandcamp_url`/`spotify_url`/`instagram_url`, but the create payload + this skill's schema use the bare `bandcamp`/`spotify`/`instagram`; and `searchArtists()` projects the existing value from a non-existent top-level `bandcamp_url` when the `/artists/search` response actually nests it under `social.bandcamp`. Net: for an artist that already exists, neither the proposed nor the existing social value is seen → it stays a SKIP and the link is never filled. Only `name`/`city`/`state`/`country`/`website`/`description` (whose names align on both sides) enrich on re-ingest. **Workarounds until PSY-1170 lands:** real duplicate missing a link → `PATCH /admin/artists/{id} {"bandcamp":…}`; false-match victim → pre-create it *with* the field (step 5). Newly-created artists are unaffected — their `bandcamp` persists fine.
