...
**show**: `event_date`, `city`, `state`, `title`, `price`, `door_price`, `doors_at`, `music_at`, `ticket_url`, `artists`, `venues` (`doors_at`/`music_at` are venue-local wall clocks and only when the source LABELS which time it is)  
