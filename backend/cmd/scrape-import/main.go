...
func main() {
	...
	if *inputPattern == "" {
		fmt.Println("Scrape Importer - Import scraped venue events into the database")
		fmt.Println()
		fmt.Println("Usage:")
		fmt.Println("  scrape-import -input <file.json> [-env <envfile>] [-dry-run] [-verbose]")
		fmt.Println()
		fmt.Println("Options:")
		...
		fmt.Println("  scrape-import -input ./output/scraped-events-2026-01-21.json")
		fmt.Println("  scrape-import -input './output/scraped-*.json' -dry-run")
		fmt.Println("  scrape-import -input ./output/events.json -env .env.production")
		os.Exit(1)
	}
	...
	for _, file := range files {
		...
		result, err := scraperService.ImportFromJSON(file, *dryRun)
		if err != nil {
