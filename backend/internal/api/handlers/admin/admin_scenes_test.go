...
func taglineHandler(t *testing.T, svcErr error) (*AdminSceneHandler, *taglineCapture) {
	...
	scene := &testhelpers.MockSceneService{
		UpdateSceneTaglineFn: func(slug string, tagline *string) (uint, error) {
			cap.called = true
			cap.slug = slug
			cap.tagline = tagline
			if svcErr != nil {
	...
	audit := &testhelpers.MockAuditLogService{
		LogEntityEditFn: func(actorID uint, entityType string, entityID uint, meta map[string]interface{}) {
			cap.audited = true
			cap.auditActor = actorID
			cap.auditEntity = entityType
			cap.auditID = entityID
			cap.auditMeta = meta
		},
	}
	return NewAdminSceneHandler(scene, audit), cap
}
...
func TestUpdateSceneTaglineHandler_SetsTagline(t *testing.T) {
	...
	if cap.tagline == nil || *cap.tagline != "Where the desert learns to scream" {
		t.Fatalf("service got tagline %v, want the authored line", cap.tagline)
	}
...
func TestUpdateSceneTaglineHandler_ClearingSpellings(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if cap.tagline != nil {
				t.Fatalf("service got %q, want nil (clear)", *cap.tagline)
			}
			if resp.Body.Tagline != nil {
				t.Fatalf("response echoed %q, want null", *resp.Body.Tagline)
			}
			if cleared, _ := cap.auditMeta["cleared"].(bool); !cleared {
				t.Fatalf("audit metadata cleared = %v, want true", cap.auditMeta["cleared"])
			}
		})
	}
}
...
// cannot smuggle leading padding past the length cap or into the markup.
func TestUpdateSceneTaglineHandler_TrimsSurroundingWhitespace(t *testing.T) {
	...
	if cap.tagline == nil || *cap.tagline != "Desert noise" {
		t.Fatalf("service got %v, want the trimmed line", cap.tagline)
	}
}
...
func TestUpdateSceneTaglineHandler_LengthGuard(t *testing.T) {
	for _, tc := range []struct {
		...
		{"at the cap", strings.Repeat("a", MaxSceneTaglineLength), true},
		{"one over the cap", strings.Repeat("a", MaxSceneTaglineLength+1), false},
		{"multibyte at the cap", strings.Repeat("é", MaxSceneTaglineLength), true},
		{"multibyte over the cap", strings.Repeat("é", MaxSceneTaglineLength+1), false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			h, cap := taglineHandler(t, nil)
			ctx := testhelpers.CtxWithUser(&authm.User{ID: 7, IsAdmin: true})
			...
			if tc.wantCall {
				if err != nil {
					t.Fatalf("unexpected error for a tagline within the cap: %v", err)
				}
				return
			}
			if err == nil {
...
func TestUpdateSceneTaglineHandler_AuditsTheEntityEdit(t *testing.T) {
	...
	if cap.auditActor != 7 {
		t.Fatalf("audit actor = %d, want 7", cap.auditActor)
	}
	if cap.auditEntity != "scene" {
		t.Fatalf("audit entity type = %q, want scene", cap.auditEntity)
	}
	if cap.auditID != 42 {
		t.Fatalf("audit entity id = %d, want the scene id the service returned (42)", cap.auditID)
	}
	if field, _ := cap.auditMeta["field"].(string); field != "tagline" {
		t.Fatalf("audit metadata field = %v, want tagline", cap.auditMeta["field"])
	}
}
