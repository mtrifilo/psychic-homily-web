...
// The composite's ALLOWLIST arm, run by Postgres rather than read (PSY-1987).
//
...
func TestVisibleCommentEntitySQLDropsUnregisteredRows(t *testing.T) {
	...
	viewers := []struct {
		...
		// The admin case is the one that used to be a blanket bypass. If any
		// spelling reintroduces `if viewer.IsAdmin { return "TRUE" }` on the
		// composite, an unregistered row starts passing again and this fails.
