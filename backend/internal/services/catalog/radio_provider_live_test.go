...
// TestWFMUFetchLiveNowPlaying_RealRebroadcastFixture parses the REAL overnight capture
// (PSY-1253; 2026-06-28 03:58 ET) where the main 91.1 block re-serves Saturday's "Freeform
// Jazz Dance" (program F4) WITH its /playlists/shows link during Sunday's automation slot.
// The parser correctly extracts it as a live payload (the link IS in .linkssection) — the
// rebroadcast SUPPRESSION is layered on at the now-playing service (isWFMUFlagshipRebroadcast),
// tested in the integration suite. This proves the captured-evidence premise: the main block
// does NOT go link-less during automation; it rebroadcasts an off-slot show with a link.
func TestWFMUFetchLiveNowPlaying_RealRebroadcastFixture(t *testing.T) {
	server := newWFMULiveServer(t, "wfmu_currentliveshows_rebroadcast.html")
	defer server.Close()
	...
	main, err := provider.FetchLiveNowPlaying(wfmuLiveChannelMain)
	require.NoError(t, err)
	require.NotNil(t, main, "main block carries a playlist link in .linkssection → parsed as live")
	assert.Equal(t, "Freeform Jazz Dance", main.ShowName)
	require.NotNil(t, main.HostName)
	assert.Equal(t, "Jason Isaac", *main.HostName)
	require.NotNil(t, main.ShowExternalID)
	assert.Equal(t, "F4", *main.ShowExternalID)

	// The pure-automation side streams (a song line only — no program, no playlist link)
	// report not-live, exactly as the link rule intends.
	drummer, err := provider.FetchLiveNowPlaying(wfmuLiveChannelDrummer)
	require.NoError(t, err)
	assert.Nil(t, drummer, "Give the Drummer has no playlist link in the capture → not live")
	sheena, err := provider.FetchLiveNowPlaying(wfmuLiveChannelSheena)
	require.NoError(t, err)
	assert.Nil(t, sheena, "Sheena's has no playlist link in the capture → not live")
}

// wfmuAggregatorStrayLinkOutsideLinkssectionHTML puts the ONLY /playlists/shows link in a
// non-.linkssection div (a "previous show" href); the live-DJ .linkssection carries only an
// MP3 link. The block must read not-live — a stray playlist href elsewhere must not force a
// false-live (PSY-1253 link-gate hardening).
const wfmuAggregatorStrayLinkOutsideLinkssectionHTML = `
<div id="nowplaysong">
<div id="nowplaying">
<div class="item-even">
<div class="streamtitle">
...
<div class="previousshow">
<a href="/playlists/shows/165000">Previous show's playlist</a>
</div>
<div class="linkssection">
<span class="links">
<a href="/wfmu.pls">128k MP3</a>
...
func TestWFMUFetchLiveNowPlaying_LinkOutsideLinkssectionNotLive(t *testing.T) {
	server := newWFMULiveServerRaw(t, []byte(wfmuAggregatorStrayLinkOutsideLinkssectionHTML))
	defer server.Close()
	...
	main, err := provider.FetchLiveNowPlaying(wfmuLiveChannelMain)
	require.NoError(t, err)
	assert.Nil(t, main, "a /playlists/shows link outside .linkssection must not force live")
}
