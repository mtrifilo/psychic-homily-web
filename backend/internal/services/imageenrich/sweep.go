...
// run state, so a deploy no longer resets the clock: this sweep was one of several
// that ran exactly ONE cycle in the life of production because the old interval
// ticker was anchored at process start.
...
func (s *ImageEnrichmentSweep) runCycle(ctx context.Context) {
	artists, err := s.sweep(ctx, "artists", s.enricher.EnrichPhotos)
	if ctx.Err() != nil {
		shared.ReportCycle(ctx, artists, err)
		return
	}
	releases, releaseErr := s.sweep(ctx, "releases", s.enricher.EnrichCovers)
	if err == nil {
		err = releaseErr
	}
	shared.ReportCycle(ctx, artists+releases, err)
}
...
// It returns how many rows it attempted and the first failure, so the caller can
// put both into the loop's durable run trace; the logging behaviour is unchanged.
func (s *ImageEnrichmentSweep) sweep(ctx context.Context, table string, enrich func(context.Context, []uint) error) (int, error) {
