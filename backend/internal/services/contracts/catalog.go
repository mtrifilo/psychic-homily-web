...
type VenueWithShowCountResponse struct {
	...
	//
	// Bounded at the REQUEST INSTANT, unlike NextShow above it, which is bounded
	// at the venue-local night. A set already under way is absent here and
	// present there.
...
type ShowAlsoTonightResponse struct {
	...
	// can say so rather than implying it listed everything. Which rows the cap
	// drops follows the order below: the latest of the date, and on the live night
	// the started ones before any that are still to come. A client that needs the
	// full night follows SceneSlug + Date.
	...
	// keeps clock order. That rule is applied before the cap, so no show still to
	// come is dropped in favour of one that has started.
	//
	// On a night that overflows the cap this is what the started rows pay: they are
	// dropped first, and a rail holding nothing but started shows is capped
	// earliest-first like any other date.
	//
...
//
// A crew is a tag, not an entity (tags.category is crew), so the payload is the
// tag's own identity plus one derived number. Slug is what the chip links to.
//
// ShowCount is scene-LOCAL: shows at venues in this scene's scope only, so the
// same crew reports a different number on every scene page it appears on. It is
// never the tag's global usage_count, which counts every entity type on every
// surface and would rank a national promoter above the crew that actually books
// this town.
