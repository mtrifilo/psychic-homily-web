...
docker compose -f "backend/docker-compose.prod.yml" --env-file "backend/.env.production" down --remove-orphans 2>/dev/null || true
...
if docker container inspect ph_production_db >/dev/null 2>&1; then
    echo "🔄 Stopping existing database container gracefully..."
    docker container stop ph_production_db 2>/dev/null || true
    docker container rm ph_production_db 2>/dev/null || true
fi
...
docker compose -f "backend/docker-compose.prod.yml" --env-file "backend/.env.production" up -d db redis

# Wait for database health with better error handling
echo "⏳ Waiting for production database..."
DB_READY=false
for i in {1..20}; do
    if docker compose -f "backend/docker-compose.prod.yml" --env-file "backend/.env.production" exec -T db pg_isready -U "${POSTGRES_USER:-postgres}" -d "${POSTGRES_DB:-postgres}" >/dev/null 2>&1; then
...
if ! docker compose -f "backend/docker-compose.prod.yml" --env-file "backend/.env.production" run --rm migrate; then
    echo "❌ Production migration failed - aborting deployment"
    docker compose -f "backend/docker-compose.prod.yml" --env-file "backend/.env.production" logs migrate
    exit 1
fi
