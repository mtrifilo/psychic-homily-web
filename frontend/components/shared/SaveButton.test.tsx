...
describe('SaveButton', () => {
  ...
  it('shows the public save count to anonymous visitors', () => {
    anonymous()
    render(
      <SaveButton showId={1} saveData={{ save_count: 7, is_saved: false }} />
    )
    expect(screen.getByText('7')).toBeInTheDocument()
    expect(
      screen.getByLabelText('Sign in to save (7 saved)')
    ).toBeInTheDocument()
  })
  ...
  it('redirects anonymous visitors to sign-in instead of toggling', async () => {
    ...
    anonymous()
    window.history.replaceState({}, '', '/?window=month')
    render(<SaveButton showId={1} />)
  ...
  it('renders the save count from saveData without fetching its own', () => {
    render(
      <SaveButton showId={1} saveData={{ save_count: 12, is_saved: false }} />
    )
  ...
  it('passes showId and the resolved isSaved to useSaveShowToggle', () => {
    render(
      <SaveButton showId={42} saveData={{ save_count: 2, is_saved: true }} />
    )
    expect(mockUseSaveShowToggle).toHaveBeenCalledWith(42, true, 42)
  })

  it('defaults isSaved to false while the count query is still loading', () => {
    render(<SaveButton showId={42} />)
    expect(mockUseSaveShowToggle).toHaveBeenCalledWith(42, false, 42)
  })
  ...
  it('leaves no pending error timer behind on unmount', async () => {
    ...
    try {
      ...
      const { unmount } = render(<SaveButton showId={1} />)
