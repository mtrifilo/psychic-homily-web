...
- **Migrations**: SQL files in `db/migrations/`. New files use timestamp versioning (`YYYYMMDDhhmmss_name.up.sql` / `.down.sql`); `000001`–`000077` remain on the original sequential scheme. Full rules, historical exceptions, and `CREATE INDEX CONCURRENTLY` guidance: `docs/strategy/migrations.md`.
...
- **Worker-scoped fixture cleanup** (PSY-432): every worker's seeded user has its mutable state wiped via `POST /admin/test-fixtures/reset` when the Playwright worker tears down (`e2e/fixtures/auth.ts` `workerCleanup`). Auto-fires even after a test crash, so mid-test failures don't leak into later runs. Scopes: `user_bookmarks` (saves/favorites/follows/going/interested), `collection_items`, `collection_subscribers`, `collections`, `pending_shows`. Comments/field-notes clean up via direct DELETE inside each test — keep it that way. The reset endpoint is layered-defended: only registered when `ENABLE_TEST_FIXTURES=1`, backend refuses to boot if that flag is on and `ENVIRONMENT` is not in `{test, ci, development}`, requires admin JWT + `X-Test-Fixtures: 1` header + target user email ending in `@test.local`. If you add a new user-FK table, either extend `testFixtureAllowlist` (backend/internal/api/handlers/test_fixtures.go) or add it to `testFixturesAllowlistContractSkips` (test_fixtures_allowlist_test.go) — the contract test fails CI on drift.
- **E2E suite ops** (PSY-436): time budget, flake policy, profiling, per-worker user model, cleanup model — `docs/strategy/e2e-testing.md`. Read this before touching an E2E test that isn't a straight feature add.
